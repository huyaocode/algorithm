...
var horsMap = [], boyeMap = [], boye2Map = [];
var horsCount = 1000, boyeCount = 1000, boye2Count = 1000;
 ...
 */
while(horsCount > 0 || boyeCount > 0 || boye2Count > 0){
    ...
    if(boyerMoorePT.pos != -1 && boyeMap[boyerMoorePT.approximatePos / 100] == 0){
        boyeMatchArr.push(boyerMoorePT);
        boyeCount--;
        boyeMap[boyerMoorePT.approximatePos / 100] = 1;
    }
    console.log('boyerMoore  ', boyerMoorePT.pos)

    var boyerMoore2PT = helper.speedTest(boyerMoore2, pattern, text);
     if(boyerMoore2PT.pos != -1 && boye2Map[boyerMoore2PT.approximatePos / 100] == 0){
        boye2MatchArr.push(boyerMoore2PT);
        boye2Count--;
        boye2Map[boyerMoore2PT.approximatePos / 100] = 1;
    }
